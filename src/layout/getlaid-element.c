...
#define COORD(bound, dir) \
  (*(dir == GTLD_ROW ? &(bound)->x : &(bound)->y))
#define SIZE(bound, dir) \
  (*(dir == GTLD_ROW ? &(bound)->w : &(bound)->h))

static int set_main_axis_sizes(
  ResizingState* state, const GTLD_Axis* main,
  GTLD_Direction dir
) {
  GTLD_Rect** auto_target_ptrs =
    malloc(sizeof(GTLD_Rect*) * state->count);
  int auto_count = 0;
  ...
  for (int i = 0; i < state->count; ++i) {
    ...
    SIZE(&state->rects[i], dir) = GTLD_UnitToPX(
      ...
      SIZE(state->parent_rect, dir), 0
    );
    state->free_space_main -= SIZE(&state->rects[i], dir);
  }

  for (int i = 0; i < auto_count; ++i) {
    SIZE(auto_target_ptrs[i], dir) =
      state->free_space_main / auto_count;
  }
...
static int set_main_axis_coords(
  ResizingState* state, GTLD_Alignment main_align,
  GTLD_Direction dir
) {
  COORD(&state->rects[0], dir) =
    COORD(state->parent_rect, dir);

  switch (main_align) {
    case GTLD_ALIGN_BEGIN:
      break;
    case GTLD_ALIGN_CENTER:
      COORD(&state->rects[0], dir) +=
        state->free_space_main / 2;
      break;
    case GTLD_ALIGN_END:
      COORD(&state->rects[0], dir) +=
        state->free_space_main;
      break;
  }

  for (int i = 1; i < state->count; ++i) {
    COORD(&state->rects[i], dir) =
      COORD(&state->rects[i - 1], dir);
    COORD(&state->rects[i], dir) +=
      SIZE(&state->rects[i - 1], dir);
  }
...
static int set_cross_axis_sizes(
  ResizingState* state, const GTLD_Axis* cross,
  GTLD_Direction dir
) {
  for (int i = 0; i < state->count; ++i) {
    SIZE(&state->rects[i], dir) = GTLD_UnitToPX(
...
static int set_cross_axis_coords(
  ResizingState* state, GTLD_Alignment align,
  GTLD_Direction dir
) {
  for (int i = 0; i < state->count; ++i) {
    COORD(&state->rects[i], dir) =
    ...
    switch (align) {
      case GTLD_ALIGN_BEGIN:
        break;
      case GTLD_ALIGN_CENTER:
        offset = SIZE(state->parent_rect, dir) -
                 SIZE(&state->rects[i], dir);
        offset = offset / 2;
...
GTLD_Rect* GTLD_GetChildrenBounds(
  const GTLD_Container* c, const GTLD_Rect* parent_rect,
  int child_count
) {
  ResizingState state = {
  ...
  const GTLD_Direction cross_dir =
    (main_dir == GTLD_ROW) ? GTLD_COLUMN : GTLD_ROW;
